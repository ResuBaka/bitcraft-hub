...
pub struct StorageLogStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> StorageLogStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ActionLogState]) + Send + 'static,
    ) -> StorageLogStateInitialCallbackId {
        StorageLogStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: StorageLogStateInitialCallbackId) {
