...
pub struct WaystoneStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> WaystoneStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[WaystoneState]) + Send + 'static,
    ) -> WaystoneStateInitialCallbackId {
        WaystoneStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: WaystoneStateInitialCallbackId) {
