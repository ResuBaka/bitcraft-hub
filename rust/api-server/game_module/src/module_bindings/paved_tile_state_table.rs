...
pub struct PavedTileStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PavedTileStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PavedTileState]) + Send + 'static,
    ) -> PavedTileStateInitialCallbackId {
        PavedTileStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PavedTileStateInitialCallbackId) {
