...
pub struct HideDeployableTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> HideDeployableTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[HideDeployableTimer]) + Send + 'static,
    ) -> HideDeployableTimerInitialCallbackId {
        HideDeployableTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: HideDeployableTimerInitialCallbackId) {
