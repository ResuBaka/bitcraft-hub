...
pub struct ConfigInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ConfigTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[Config]) + Send + 'static,
    ) -> ConfigInitialCallbackId {
        ConfigInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ConfigInitialCallbackId) {
...
impl<'ctx> __sdk::Table for ConfigTableHandle<'ctx> {
    ...
    __sdk::__if_client_cache! {
        ...
        fn iter(&self) -> impl Iterator<Item = Config> + '_ { self.imp.iter() }
    }
