...
pub struct UnclaimedShardsStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> UnclaimedShardsStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[UnclaimedShardsState]) + Send + 'static,
    ) -> UnclaimedShardsStateInitialCallbackId {
        UnclaimedShardsStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: UnclaimedShardsStateInitialCallbackId) {
