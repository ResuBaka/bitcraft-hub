...
pub struct PlayerReportStateTimestampInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PlayerReportStateTimestampTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PlayerReportStateTimestamp]) + Send + 'static,
    ) -> PlayerReportStateTimestampInitialCallbackId {
        PlayerReportStateTimestampInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PlayerReportStateTimestampInitialCallbackId) {
