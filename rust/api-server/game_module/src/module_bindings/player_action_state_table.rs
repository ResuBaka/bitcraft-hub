...
pub struct PlayerActionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PlayerActionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PlayerActionState]) + Send + 'static,
    ) -> PlayerActionStateInitialCallbackId {
        PlayerActionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PlayerActionStateInitialCallbackId) {
