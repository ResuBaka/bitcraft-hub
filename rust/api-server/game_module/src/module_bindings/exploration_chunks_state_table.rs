...
pub struct ExplorationChunksStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ExplorationChunksStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ExplorationChunksState]) + Send + 'static,
    ) -> ExplorationChunksStateInitialCallbackId {
        ExplorationChunksStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ExplorationChunksStateInitialCallbackId) {
