...
impl<'ctx> ExtractOutcomeStateV1TableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ExtractOutcomeState]) + Send + 'static,
    ) -> ExtractOutcomeStateV1InitialCallbackId {
        ExtractOutcomeStateV1InitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ExtractOutcomeStateV1InitialCallbackId) {
