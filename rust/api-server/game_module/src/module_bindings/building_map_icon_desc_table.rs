...
pub struct BuildingMapIconDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> BuildingMapIconDescTableHandle<'ctx> {
    ...
    ) -> BuildingMapIconDescInitialCallbackId {
        BuildingMapIconDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: BuildingMapIconDescInitialCallbackId) {
