...
pub struct DimensionDescriptionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> DimensionDescriptionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[DimensionDescriptionState]) + Send + 'static,
    ) -> DimensionDescriptionStateInitialCallbackId {
        DimensionDescriptionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DimensionDescriptionStateInitialCallbackId) {
