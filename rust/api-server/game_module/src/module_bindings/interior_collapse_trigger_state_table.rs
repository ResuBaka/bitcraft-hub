...
impl<'ctx> InteriorCollapseTriggerStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[InteriorCollapseTriggerState]) + Send + 'static,
    ) -> InteriorCollapseTriggerStateInitialCallbackId {
        InteriorCollapseTriggerStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: InteriorCollapseTriggerStateInitialCallbackId) {
