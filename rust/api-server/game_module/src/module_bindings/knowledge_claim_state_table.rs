...
pub struct KnowledgeClaimStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgeClaimStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgeClaimState]) + Send + 'static,
    ) -> KnowledgeClaimStateInitialCallbackId {
        KnowledgeClaimStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeClaimStateInitialCallbackId) {
