...
pub struct StagedParametersDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> StagedParametersDescTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ParametersDesc]) + Send + 'static,
    ) -> StagedParametersDescInitialCallbackId {
        StagedParametersDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: StagedParametersDescInitialCallbackId) {
