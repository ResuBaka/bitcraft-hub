...
pub struct TradeOrderStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TradeOrderStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[TradeOrderState]) + Send + 'static,
    ) -> TradeOrderStateInitialCallbackId {
        TradeOrderStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TradeOrderStateInitialCallbackId) {
