...
pub struct DestroyDimensionNetworkTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> DestroyDimensionNetworkTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[DestroyDimensionNetworkTimer]) + Send + 'static,
    ) -> DestroyDimensionNetworkTimerInitialCallbackId {
        DestroyDimensionNetworkTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DestroyDimensionNetworkTimerInitialCallbackId) {
