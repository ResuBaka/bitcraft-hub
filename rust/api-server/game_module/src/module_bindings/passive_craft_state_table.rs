...
impl<'ctx> PassiveCraftStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PassiveCraftState]) + Send + 'static,
    ) -> PassiveCraftStateInitialCallbackId {
        PassiveCraftStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PassiveCraftStateInitialCallbackId) {
