...
pub struct EnemyMobMonitorStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EnemyMobMonitorStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[EnemyMobMonitorState]) + Send + 'static,
    ) -> EnemyMobMonitorStateInitialCallbackId {
        EnemyMobMonitorStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EnemyMobMonitorStateInitialCallbackId) {
