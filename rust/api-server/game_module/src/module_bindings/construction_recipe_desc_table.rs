...
pub struct ConstructionRecipeDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ConstructionRecipeDescTableHandle<'ctx> {
    ...
    ) -> ConstructionRecipeDescInitialCallbackId {
        ConstructionRecipeDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ConstructionRecipeDescInitialCallbackId) {
