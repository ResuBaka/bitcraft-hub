...
pub struct QuestChainDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> QuestChainDescTableHandle<'ctx> {
    ...
    ) -> QuestChainDescInitialCallbackId {
        QuestChainDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: QuestChainDescInitialCallbackId) {
