...
pub struct TradeSessionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TradeSessionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[TradeSessionState]) + Send + 'static,
    ) -> TradeSessionStateInitialCallbackId {
        TradeSessionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TradeSessionStateInitialCallbackId) {
