...
impl<'ctx> ActiveBuffStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ActiveBuffState]) + Send + 'static,
    ) -> ActiveBuffStateInitialCallbackId {
        ActiveBuffStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ActiveBuffStateInitialCallbackId) {
