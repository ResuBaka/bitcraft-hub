...
pub struct AbilityStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> AbilityStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[AbilityState]) + Send + 'static,
    ) -> AbilityStateInitialCallbackId {
        AbilityStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: AbilityStateInitialCallbackId) {
