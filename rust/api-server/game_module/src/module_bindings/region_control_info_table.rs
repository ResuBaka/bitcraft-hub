...
pub struct RegionControlInfoInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> RegionControlInfoTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[RegionControlInfo]) + Send + 'static,
    ) -> RegionControlInfoInitialCallbackId {
        RegionControlInfoInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: RegionControlInfoInitialCallbackId) {
