...
pub struct EquipmentPresetStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EquipmentPresetStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[EquipmentPresetState]) + Send + 'static,
    ) -> EquipmentPresetStateInitialCallbackId {
        EquipmentPresetStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EquipmentPresetStateInitialCallbackId) {
