...
pub struct EnemyRegenLoopTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EnemyRegenLoopTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[EnemyRegenLoopTimer]) + Send + 'static,
    ) -> EnemyRegenLoopTimerInitialCallbackId {
        EnemyRegenLoopTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EnemyRegenLoopTimerInitialCallbackId) {
