...
pub struct ResourcesLogInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ResourcesLogTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ResourcesLog]) + Send + 'static,
    ) -> ResourcesLogInitialCallbackId {
        ResourcesLogInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ResourcesLogInitialCallbackId) {
