...
pub struct ExperienceStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ExperienceStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ExperienceState]) + Send + 'static,
    ) -> ExperienceStateInitialCallbackId {
        ExperienceStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ExperienceStateInitialCallbackId) {
