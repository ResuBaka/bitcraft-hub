...
pub struct DroppedInventoryDespawnTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> DroppedInventoryDespawnTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[DroppedInventoryDespawnTimer]) + Send + 'static,
    ) -> DroppedInventoryDespawnTimerInitialCallbackId {
        DroppedInventoryDespawnTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DroppedInventoryDespawnTimerInitialCallbackId) {
