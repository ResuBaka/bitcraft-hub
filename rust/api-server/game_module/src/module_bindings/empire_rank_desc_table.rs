...
pub struct EmpireRankDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EmpireRankDescTableHandle<'ctx> {
    ...
    ) -> EmpireRankDescInitialCallbackId {
        EmpireRankDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EmpireRankDescInitialCallbackId) {
