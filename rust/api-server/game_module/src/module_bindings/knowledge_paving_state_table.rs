...
pub struct KnowledgePavingStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgePavingStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgePavingState]) + Send + 'static,
    ) -> KnowledgePavingStateInitialCallbackId {
        KnowledgePavingStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgePavingStateInitialCallbackId) {
