...
pub struct DeployableStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> DeployableStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[DeployableState]) + Send + 'static,
    ) -> DeployableStateInitialCallbackId {
        DeployableStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DeployableStateInitialCallbackId) {
