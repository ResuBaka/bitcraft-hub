...
pub struct ResourceHealthStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ResourceHealthStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ResourceHealthState]) + Send + 'static,
    ) -> ResourceHealthStateInitialCallbackId {
        ResourceHealthStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ResourceHealthStateInitialCallbackId) {
