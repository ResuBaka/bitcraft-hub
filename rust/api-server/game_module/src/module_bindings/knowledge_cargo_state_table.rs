...
impl<'ctx> KnowledgeCargoStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgeCargoState]) + Send + 'static,
    ) -> KnowledgeCargoStateInitialCallbackId {
        KnowledgeCargoStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeCargoStateInitialCallbackId) {
