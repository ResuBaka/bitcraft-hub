...
pub struct CombatDimensionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> CombatDimensionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[CombatDimensionState]) + Send + 'static,
    ) -> CombatDimensionStateInitialCallbackId {
        CombatDimensionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: CombatDimensionStateInitialCallbackId) {
