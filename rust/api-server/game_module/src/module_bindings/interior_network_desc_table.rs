...
pub struct InteriorNetworkDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> InteriorNetworkDescTableHandle<'ctx> {
    ...
    ) -> InteriorNetworkDescInitialCallbackId {
        InteriorNetworkDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: InteriorNetworkDescInitialCallbackId) {
