...
pub struct ClaimTechStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ClaimTechStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ClaimTechState]) + Send + 'static,
    ) -> ClaimTechStateInitialCallbackId {
        ClaimTechStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ClaimTechStateInitialCallbackId) {
