...
pub struct KnowledgeRuinsStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgeRuinsStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgeRuinsState]) + Send + 'static,
    ) -> KnowledgeRuinsStateInitialCallbackId {
        KnowledgeRuinsStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeRuinsStateInitialCallbackId) {
