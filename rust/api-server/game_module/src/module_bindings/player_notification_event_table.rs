...
pub struct PlayerNotificationEventInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PlayerNotificationEventTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PlayerNotificationEvent]) + Send + 'static,
    ) -> PlayerNotificationEventInitialCallbackId {
        PlayerNotificationEventInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PlayerNotificationEventInitialCallbackId) {
