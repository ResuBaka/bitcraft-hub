...
impl<'ctx> ActionBarStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ActionBarState]) + Send + 'static,
    ) -> ActionBarStateInitialCallbackId {
        ActionBarStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ActionBarStateInitialCallbackId) {
