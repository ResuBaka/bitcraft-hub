...
pub struct TeleportationEnergyRegenLoopTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TeleportationEnergyRegenLoopTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[TeleportationEnergyRegenLoopTimer])
        + Send
        + 'static,
    ) -> TeleportationEnergyRegenLoopTimerInitialCallbackId {
        TeleportationEnergyRegenLoopTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TeleportationEnergyRegenLoopTimerInitialCallbackId) {
