...
pub struct ResetChunkIndexTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ResetChunkIndexTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ResetChunkIndexTimer]) + Send + 'static,
    ) -> ResetChunkIndexTimerInitialCallbackId {
        ResetChunkIndexTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ResetChunkIndexTimerInitialCallbackId) {
