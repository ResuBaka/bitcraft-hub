...
pub struct EmpireColorDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EmpireColorDescTableHandle<'ctx> {
    ...
    ) -> EmpireColorDescInitialCallbackId {
        EmpireColorDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EmpireColorDescInitialCallbackId) {
