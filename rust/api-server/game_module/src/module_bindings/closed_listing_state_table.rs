...
impl<'ctx> ClosedListingStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ClosedListingState]) + Send + 'static,
    ) -> ClosedListingStateInitialCallbackId {
        ClosedListingStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ClosedListingStateInitialCallbackId) {
