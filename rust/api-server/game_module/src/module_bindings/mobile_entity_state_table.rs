...
pub struct MobileEntityStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> MobileEntityStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[MobileEntityState]) + Send + 'static,
    ) -> MobileEntityStateInitialCallbackId {
        MobileEntityStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: MobileEntityStateInitialCallbackId) {
