...
pub struct PlayerDeathTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PlayerDeathTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PlayerDeathTimer]) + Send + 'static,
    ) -> PlayerDeathTimerInitialCallbackId {
        PlayerDeathTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PlayerDeathTimerInitialCallbackId) {
