...
pub struct TerrainChunkStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TerrainChunkStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[TerrainChunkState]) + Send + 'static,
    ) -> TerrainChunkStateInitialCallbackId {
        TerrainChunkStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TerrainChunkStateInitialCallbackId) {
