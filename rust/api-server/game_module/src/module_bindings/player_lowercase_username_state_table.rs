...
pub struct PlayerLowercaseUsernameStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PlayerLowercaseUsernameStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PlayerLowercaseUsernameState]) + Send + 'static,
    ) -> PlayerLowercaseUsernameStateInitialCallbackId {
        PlayerLowercaseUsernameStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PlayerLowercaseUsernameStateInitialCallbackId) {
