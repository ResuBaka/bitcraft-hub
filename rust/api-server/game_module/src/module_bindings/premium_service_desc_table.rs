...
pub struct PremiumServiceDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PremiumServiceDescTableHandle<'ctx> {
    ...
    ) -> PremiumServiceDescInitialCallbackId {
        PremiumServiceDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PremiumServiceDescInitialCallbackId) {
