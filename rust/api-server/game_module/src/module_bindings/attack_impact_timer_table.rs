...
pub struct AttackImpactTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> AttackImpactTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[AttackImpactTimer]) + Send + 'static,
    ) -> AttackImpactTimerInitialCallbackId {
        AttackImpactTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: AttackImpactTimerInitialCallbackId) {
