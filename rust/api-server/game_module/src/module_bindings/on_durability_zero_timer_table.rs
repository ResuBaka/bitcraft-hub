...
pub struct OnDurabilityZeroTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> OnDurabilityZeroTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[OnDurabilityZeroTimer]) + Send + 'static,
    ) -> OnDurabilityZeroTimerInitialCallbackId {
        OnDurabilityZeroTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: OnDurabilityZeroTimerInitialCallbackId) {
