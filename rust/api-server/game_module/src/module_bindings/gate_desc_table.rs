...
pub struct GateDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> GateDescTableHandle<'ctx> {
    ...
    ) -> GateDescInitialCallbackId {
        GateDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: GateDescInitialCallbackId) {
