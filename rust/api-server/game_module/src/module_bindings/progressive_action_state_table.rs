...
pub struct ProgressiveActionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ProgressiveActionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ProgressiveActionState]) + Send + 'static,
    ) -> ProgressiveActionStateInitialCallbackId {
        ProgressiveActionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ProgressiveActionStateInitialCallbackId) {
