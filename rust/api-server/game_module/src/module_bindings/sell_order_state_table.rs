...
pub struct SellOrderStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> SellOrderStateTableHandle<'ctx> {
    ...
    ) -> SellOrderStateInitialCallbackId {
        SellOrderStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: SellOrderStateInitialCallbackId) {
