...
pub struct TransferPlayerTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TransferPlayerTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[TransferPlayerTimer]) + Send + 'static,
    ) -> TransferPlayerTimerInitialCallbackId {
        TransferPlayerTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TransferPlayerTimerInitialCallbackId) {
