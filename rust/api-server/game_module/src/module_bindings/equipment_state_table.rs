...
pub struct EquipmentStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EquipmentStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[EquipmentState]) + Send + 'static,
    ) -> EquipmentStateInitialCallbackId {
        EquipmentStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EquipmentStateInitialCallbackId) {
