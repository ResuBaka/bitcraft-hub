...
impl<'ctx> DeployableStateV2TableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[DeployableStateV2]) + Send + 'static,
    ) -> DeployableStateV2InitialCallbackId {
        DeployableStateV2InitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DeployableStateV2InitialCallbackId) {
