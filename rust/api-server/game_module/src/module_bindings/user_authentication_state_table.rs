...
pub struct UserAuthenticationStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> UserAuthenticationStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[UserAuthenticationState]) + Send + 'static,
    ) -> UserAuthenticationStateInitialCallbackId {
        UserAuthenticationStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: UserAuthenticationStateInitialCallbackId) {
...
#[allow(non_camel_case_types)]
/// Extension trait for query builder access to the table `UserAuthenticationState`.
///
/// Implemented for [`__sdk::QueryTableAccessor`].
pub trait user_authentication_stateQueryTableAccess {
    #[allow(non_snake_case)]
    /// Get a query builder for the table `UserAuthenticationState`.
    fn user_authentication_state(&self) -> __sdk::__query_builder::Table<UserAuthenticationState>;
}

impl user_authentication_stateQueryTableAccess for __sdk::QueryTableAccessor {
    fn user_authentication_state(&self) -> __sdk::__query_builder::Table<UserAuthenticationState> {
        __sdk::__query_builder::Table::new("user_authentication_state")
    }
}
