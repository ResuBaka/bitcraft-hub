...
pub struct CombatActionDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> CombatActionDescTableHandle<'ctx> {
    ...
    ) -> CombatActionDescInitialCallbackId {
        CombatActionDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: CombatActionDescInitialCallbackId) {
