...
pub struct DistantVisibleEntityDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> DistantVisibleEntityDescTableHandle<'ctx> {
    ...
    ) -> DistantVisibleEntityDescInitialCallbackId {
        DistantVisibleEntityDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DistantVisibleEntityDescInitialCallbackId) {
