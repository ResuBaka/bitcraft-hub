...
pub struct TeleportPlayerTimerInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TeleportPlayerTimerTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[TeleportPlayerTimer]) + Send + 'static,
    ) -> TeleportPlayerTimerInitialCallbackId {
        TeleportPlayerTimerInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TeleportPlayerTimerInitialCallbackId) {
