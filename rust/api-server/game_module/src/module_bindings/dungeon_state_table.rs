...
impl<'ctx> DungeonStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[DungeonState]) + Send + 'static,
    ) -> DungeonStateInitialCallbackId {
        DungeonStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DungeonStateInitialCallbackId) {
