...
pub struct InteriorPlayerCountStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> InteriorPlayerCountStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[InteriorPlayerCountState]) + Send + 'static,
    ) -> InteriorPlayerCountStateInitialCallbackId {
        InteriorPlayerCountStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: InteriorPlayerCountStateInitialCallbackId) {
