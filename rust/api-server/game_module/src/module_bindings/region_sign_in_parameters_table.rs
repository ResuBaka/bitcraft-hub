...
pub struct RegionSignInParametersInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> RegionSignInParametersTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[RegionSignInParameters]) + Send + 'static,
    ) -> RegionSignInParametersInitialCallbackId {
        RegionSignInParametersInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: RegionSignInParametersInitialCallbackId) {
