...
pub struct PlayerHousingMovingCostStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PlayerHousingMovingCostStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PlayerHousingMovingCostState]) + Send + 'static,
    ) -> PlayerHousingMovingCostStateInitialCallbackId {
        PlayerHousingMovingCostStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PlayerHousingMovingCostStateInitialCallbackId) {
