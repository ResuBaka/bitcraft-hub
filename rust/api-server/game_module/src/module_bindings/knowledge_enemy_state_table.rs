...
pub struct KnowledgeEnemyStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgeEnemyStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgeEnemyState]) + Send + 'static,
    ) -> KnowledgeEnemyStateInitialCallbackId {
        KnowledgeEnemyStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeEnemyStateInitialCallbackId) {
