...
impl<'ctx> InterModuleMessageV5TableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[InterModuleMessageV5]) + Send + 'static,
    ) -> InterModuleMessageV5InitialCallbackId {
        InterModuleMessageV5InitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: InterModuleMessageV5InitialCallbackId) {
