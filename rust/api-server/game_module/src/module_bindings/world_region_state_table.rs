...
pub struct WorldRegionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> WorldRegionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[WorldRegionState]) + Send + 'static,
    ) -> WorldRegionStateInitialCallbackId {
        WorldRegionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: WorldRegionStateInitialCallbackId) {
