...
pub struct MountingStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> MountingStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[MountingState]) + Send + 'static,
    ) -> MountingStateInitialCallbackId {
        MountingStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: MountingStateInitialCallbackId) {
