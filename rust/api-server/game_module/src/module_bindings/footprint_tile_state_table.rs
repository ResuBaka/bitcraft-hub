...
pub struct FootprintTileStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> FootprintTileStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[FootprintTileState]) + Send + 'static,
    ) -> FootprintTileStateInitialCallbackId {
        FootprintTileStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: FootprintTileStateInitialCallbackId) {
