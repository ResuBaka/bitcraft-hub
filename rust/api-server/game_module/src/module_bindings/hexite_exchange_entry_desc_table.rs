...
pub struct HexiteExchangeEntryDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> HexiteExchangeEntryDescTableHandle<'ctx> {
    ...
    ) -> HexiteExchangeEntryDescInitialCallbackId {
        HexiteExchangeEntryDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: HexiteExchangeEntryDescInitialCallbackId) {
