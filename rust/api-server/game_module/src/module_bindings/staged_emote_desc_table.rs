...
pub struct StagedEmoteDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> StagedEmoteDescTableHandle<'ctx> {
    ...
    ) -> StagedEmoteDescInitialCallbackId {
        StagedEmoteDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: StagedEmoteDescInitialCallbackId) {
