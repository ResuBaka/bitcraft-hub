...
pub struct LightSourceStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> LightSourceStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[LightSourceState]) + Send + 'static,
    ) -> LightSourceStateInitialCallbackId {
        LightSourceStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: LightSourceStateInitialCallbackId) {
