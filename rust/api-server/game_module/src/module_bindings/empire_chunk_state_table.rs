...
pub struct EmpireChunkStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EmpireChunkStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[EmpireChunkState]) + Send + 'static,
    ) -> EmpireChunkStateInitialCallbackId {
        EmpireChunkStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EmpireChunkStateInitialCallbackId) {
