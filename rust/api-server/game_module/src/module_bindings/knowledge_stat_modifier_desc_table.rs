...
pub struct KnowledgeStatModifierDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgeStatModifierDescTableHandle<'ctx> {
    ...
    ) -> KnowledgeStatModifierDescInitialCallbackId {
        KnowledgeStatModifierDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeStatModifierDescInitialCallbackId) {
