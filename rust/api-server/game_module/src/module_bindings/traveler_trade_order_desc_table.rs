...
pub struct TravelerTradeOrderDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TravelerTradeOrderDescTableHandle<'ctx> {
    ...
    ) -> TravelerTradeOrderDescInitialCallbackId {
        TravelerTradeOrderDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TravelerTradeOrderDescInitialCallbackId) {
