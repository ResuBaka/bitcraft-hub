...
pub struct KnowledgePillarShapingStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgePillarShapingStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgePillarShapingState]) + Send + 'static,
    ) -> KnowledgePillarShapingStateInitialCallbackId {
        KnowledgePillarShapingStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgePillarShapingStateInitialCallbackId) {
