...
pub struct UserPreviousRegionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> UserPreviousRegionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[UserPreviousRegionState]) + Send + 'static,
    ) -> UserPreviousRegionStateInitialCallbackId {
        UserPreviousRegionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: UserPreviousRegionStateInitialCallbackId) {
