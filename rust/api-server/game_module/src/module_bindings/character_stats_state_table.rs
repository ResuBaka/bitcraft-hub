...
pub struct CharacterStatsStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> CharacterStatsStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[CharacterStatsState]) + Send + 'static,
    ) -> CharacterStatsStateInitialCallbackId {
        CharacterStatsStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: CharacterStatsStateInitialCallbackId) {
