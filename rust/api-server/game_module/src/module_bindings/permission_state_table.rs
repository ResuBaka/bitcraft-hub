...
pub struct PermissionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> PermissionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[PermissionState]) + Send + 'static,
    ) -> PermissionStateInitialCallbackId {
        PermissionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: PermissionStateInitialCallbackId) {
