...
pub struct TravelerTaskStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> TravelerTaskStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[TravelerTaskState]) + Send + 'static,
    ) -> TravelerTaskStateInitialCallbackId {
        TravelerTaskStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: TravelerTaskStateInitialCallbackId) {
