...
pub struct EnvironmentDebuffDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> EnvironmentDebuffDescTableHandle<'ctx> {
    ...
    ) -> EnvironmentDebuffDescInitialCallbackId {
        EnvironmentDebuffDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: EnvironmentDebuffDescInitialCallbackId) {
