...
pub struct AttackOutcomeStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> AttackOutcomeStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[AttackOutcomeState]) + Send + 'static,
    ) -> AttackOutcomeStateInitialCallbackId {
        AttackOutcomeStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: AttackOutcomeStateInitialCallbackId) {
