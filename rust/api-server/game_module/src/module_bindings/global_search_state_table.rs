...
pub struct GlobalSearchStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> GlobalSearchStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[GlobalSearchState]) + Send + 'static,
    ) -> GlobalSearchStateInitialCallbackId {
        GlobalSearchStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: GlobalSearchStateInitialCallbackId) {
