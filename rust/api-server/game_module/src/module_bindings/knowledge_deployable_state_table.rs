...
pub struct KnowledgeDeployableStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgeDeployableStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgeDeployableState]) + Send + 'static,
    ) -> KnowledgeDeployableStateInitialCallbackId {
        KnowledgeDeployableStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeDeployableStateInitialCallbackId) {
