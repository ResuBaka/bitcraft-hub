...
pub struct RegionExplorationInfoInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> RegionExplorationInfoTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[RegionExplorationInfo]) + Send + 'static,
    ) -> RegionExplorationInfoInitialCallbackId {
        RegionExplorationInfoInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: RegionExplorationInfoInitialCallbackId) {
