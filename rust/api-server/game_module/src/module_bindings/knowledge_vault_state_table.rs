...
pub struct KnowledgeVaultStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgeVaultStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgeVaultState]) + Send + 'static,
    ) -> KnowledgeVaultStateInitialCallbackId {
        KnowledgeVaultStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeVaultStateInitialCallbackId) {
