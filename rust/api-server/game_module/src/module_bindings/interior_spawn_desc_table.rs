...
pub struct InteriorSpawnDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> InteriorSpawnDescTableHandle<'ctx> {
    ...
    ) -> InteriorSpawnDescInitialCallbackId {
        InteriorSpawnDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: InteriorSpawnDescInitialCallbackId) {
