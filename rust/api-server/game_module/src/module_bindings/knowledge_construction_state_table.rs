...
pub struct KnowledgeConstructionStateInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> KnowledgeConstructionStateTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[KnowledgeConstructionState]) + Send + 'static,
    ) -> KnowledgeConstructionStateInitialCallbackId {
        KnowledgeConstructionStateInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: KnowledgeConstructionStateInitialCallbackId) {
