...
pub struct ParametersPlayerMoveDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> ParametersPlayerMoveDescTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[ParametersPlayerMoveDesc]) + Send + 'static,
    ) -> ParametersPlayerMoveDescInitialCallbackId {
        ParametersPlayerMoveDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: ParametersPlayerMoveDescInitialCallbackId) {
