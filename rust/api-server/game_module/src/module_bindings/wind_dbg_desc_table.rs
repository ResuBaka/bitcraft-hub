...
pub struct WindDbgDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> WindDbgDescTableHandle<'ctx> {
    ...
    pub fn on_initial(
        &self,
        callback: impl FnMut(&super::EventContext, &[WindDbgDesc]) + Send + 'static,
    ) -> WindDbgDescInitialCallbackId {
        WindDbgDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: WindDbgDescInitialCallbackId) {
