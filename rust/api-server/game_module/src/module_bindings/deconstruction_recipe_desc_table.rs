...
pub struct DeconstructionRecipeDescInitialCallbackId(__sdk::CallbackId);
...
impl<'ctx> DeconstructionRecipeDescTableHandle<'ctx> {
    ...
    ) -> DeconstructionRecipeDescInitialCallbackId {
        DeconstructionRecipeDescInitialCallbackId(self.imp.on_initial(callback))
    }

    /// Cancel a callback previously registered by [`Self::on_initial`].
    pub fn remove_on_initial(&self, callback: DeconstructionRecipeDescInitialCallbackId) {
