...
async fn known_claim_state_ids(
...
async fn delete_claim_state(
    ...
    claim_state::Entity::delete_many()
        .filter(
            claim_state::Column::EntityId.is_in(known_claim_ids.clone()),
        )
        .exec(conn)
        .await?;
...
pub(crate) async fn handle_initial_subscription(
    ...
    let chunk_size = 5000;
    let mut buffer_before_insert: Vec<claim_state::Model> = vec![];

    let mut known_inventory_ids = known_claim_state_ids(&app_state.conn).await?;
    for update in p1.updates.iter() {
        for row in update.inserts.iter() {
            match serde_json::from_str::<claim_state::Model>(row.as_ref()) {
                Ok(claim_state) => {
                    if known_inventory_ids.contains(&claim_state.entity_id) {
                        known_inventory_ids.remove(&claim_state.entity_id);
                    }
                    buffer_before_insert.push(claim_state.clone());
                    app_state
                        .claim_state
                        .insert(claim_state.entity_id as u64, claim_state.clone());
                    if buffer_before_insert.len() == chunk_size {
                        db_insert_claim_state(
                            &app_state.conn,
    ...
    if !known_inventory_ids.is_empty() {
        delete_claim_state(&app_state.conn, known_inventory_ids).await?;
    }
...
pub(crate) async fn handle_transaction_update(
    ...
    for p1 in tables.iter() {
        ...
        if event_type == "delete" {
            for row in p1.deletes.iter() {
                match serde_json::from_str::<claim_state::Model>(row.as_ref()) {
                    Ok(claim_state) => {
                        potential_deletes.insert(claim_state.entity_id);
                    }
                    Err(error) => {
            ...
            for row in p1.deletes.iter() {
                let parsed = serde_json::from_str::<claim_state::Model>(row.as_ref());

                if parsed.is_err() {
                    tracing::error!(
                        "Could not parse delete claim_state: {}, row: {:?}",
                        parsed.unwrap_err(),
                        row
                    );
                } else {
            ...
            for row in p1.inserts.iter().enumerate() {
                let parsed = serde_json::from_str::<claim_state::Model>(row.1.as_ref());

                if parsed.is_err() {
                    tracing::error!(
                        "Could not parse insert claim_state: {}, row: {:?}",
                        parsed.unwrap_err(),
                        row.1
                    );
                    continue;
                ...
                match (parsed, delete_parsed.get(&id)) {
                    (new_claim_state, Some(_old_claim_state)) => {
                        app_state.claim_state.insert(
                            new_claim_state.entity_id as u64,
                            new_claim_state.clone(),
                        );
                        buffer_before_insert.insert(
                            new_claim_state.entity_id,
                            new_claim_state.clone(),
                        );
                        potential_deletes.remove(&new_claim_state.entity_id);
                    }
                    (_new_claim_state, None) => {
    ...
    if !potential_deletes.is_empty() {
        ...
        delete_claim_state(&app_state.conn, potential_deletes).await?;
    }
