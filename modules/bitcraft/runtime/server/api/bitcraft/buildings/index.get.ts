...
export default defineEventHandler((event) => {
    ...
    const rowsFilterted = rows?.filter((building: any) => {
        return (!claim_entity_id || building.claim_entity_id === claim_entity_id) &&
            ((!search || building.name.toLowerCase().includes(search.toLowerCase())) ||
            (!search || (building.entity_id.toString().includes(search))))
    }) ?? []
