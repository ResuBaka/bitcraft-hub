...
export function getLeaderboard(skills: SkillDescRow[], expeirence: ExpeirenceStateRow[]){
  ...
  for(const skill of skills){
    ...
    leaderboard[skill.name] = expeirenceCopy.sort((a,b) => b.experience_stacks[skill.id] - a.experience_stacks[skill.id])
  }
  return leaderboard
}
...
export function extendExperienceRowFromRow(expeirence: ExpeirenceStateRow,leaderboard: {
  ...
  for(const skill of skills){
    ...
    data[skill.name] = {
      ...
      rank: leaderboard[skill.name].findIndex((data) => data.entity_id === expeirence.entity_id) + 1
    }
  }
