...
const buyOrdersSorting = ref<SortingState>([{ id: "price_threshold", desc: true }]);
const sellOrdersSorting = ref<SortingState>([{ id: "price_threshold", desc: false }]);
const search = ref<string>("");
const selectedItemKeys = ref<string[]>([]);
const starredKeys = ref<string[]>([]);
const filterHasBuyOrders = ref(true);
const filterHasSellOrders = ref(true);
...
const { data: marketStatsData, pending: marketStatsPending } =
  await useLazyFetchMsPack<MarketOrderStatsResponse>(
    () => {
      return `/market/claim/${props.claimId}`;
    },
    { deep: false },
  );
...
const {
  ...
  () => {
    return `/market/orders/claim/${props.claimId}`;
  },
  {
    deep: false,
    immediate: false,
...
import { getPaginationRowModel, getSortedRowModel, type SortingState } from "@tanstack/vue-table";
...
const userIds = computed(() => {
  ...
  for (const sellOrder in sellOrders.value) {
    userIds.add(sellOrders.value[sellOrder].owner_entity_id.toString());
  }
  for (const buyOrder in buyOrders.value) {
    userIds.add(buyOrders.value[buyOrder].owner_entity_id.toString());
  }

  return userIds;
});

watch(userIds, (after, before) => {
  let removed = before.difference(after);

  for (const userId of removed) {
    if (usernames.value[userId]) {
      delete usernames.value[userId];
    }
  }
});

const userIdsTopics = computed(() => {
  let topics = [];

  for (const userId of userIds.value) {
    topics.push(`player_username.${userId}`);
  }

  return topics;
});

registerWebsocketMessageHandler("PlayerUsername", userIdsTopics, (message) => {
  usernames.value[message.entity_id.toString()] = message.username;
});
</script>
            ...
            <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
              <UCard :ui="{ header: 'p-4', body: 'p-0' }">
                ...
                <UTable
                  ...
                  v-model:sorting="sellOrdersSorting"
                ...
                >
                  ...
                  <template #owner_entity_id-cell="{ row }">{{
                    usernames[row.original.owner_entity_id.toString()] ||
                    row.original.owner_entity_id
                  }}</template>
              ...
              <UCard :ui="{ header: 'p-4', body: 'p-0' }">
                ...
                <UTable
                  ...
                  v-model:sorting="buyOrdersSorting"
