...
const {
  ...
  pending,
  refresh,
} = await useLazyFetch(`/api/bitcraft/empires`, {
  onRequest: ({ options }) => {
...
const currentEmpires = computed(() => {
  return empires.value?.claims ?? [];
});

const length = computed(() => {
  if (empires.value?.total) {
    return Math.ceil(empires.value?.total / perPage);
  }
...
<template>
  <v-container fluid>
    ...
    <v-row>
      <v-col cols="12" md="6" lg="4" xl="3" xxl="2" v-for="empire in currentEmpires" :key="empire.entity_id">
        <bitcraft-card-empire :empire="empire"></bitcraft-card-empire>
      </v-col>
    </v-row>
  </v-container>
</template>
